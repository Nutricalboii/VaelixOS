...
echo "✦ Phase 11: Hardening VX Command Suite (v1.1 Stable Candidate)..."
...
# We use a temporary file to avoid complex shell quoting issues
cat <<'EOF' > vx_tmp.sh
#!/bin/bash
# VX Command Suite v1.1-SC - Titanium Velocity Stable Candidate
...
case $CMD in
    update)
        ...
        if [[ "$SNAP_CHOICE" =~ ^[Yy]$ ]]; then
            ...
            sudo timeshift --create --comments "v1.1-SC Pre-Update Sync" || err "Snapshot failed!"
        fi
    ...
    report)
        ...
        {
            echo "--- VAELIX OS v1.1-SC REPORT ---"
            echo "Branding: Titanium Velocity (Stable Candidate)"
            echo "Date: $(date)"
            echo "Kernel: $(uname -a)"
    ...
    doctor)
        log "Vaelix v1.1-SC (Stable Candidate) Diagnostic:"
        echo -e "\n${CYAN}--- Headquarters Aura ---${NC}"
        echo "OS: Vaelix 1.1 Titanium Velocity (SC)"
        echo "Kernel: $(uname -r)"
    ...
    *)
        echo -e "${PURPLE}Vaelix Intelligence CLI v1.1-SC - Titanium Velocity${NC}"
        ...
        echo "  onboarding Launch first-run wizard"
        ;;
esac
EOF

sudo cp vx_tmp.sh "${CHROOT_DIR}/usr/local/bin/vx"
run_chroot_raw "chmod +x /usr/local/bin/vx"
rm vx_tmp.sh

echo "✦ Vaelix OS: Titanium Velocity VX (SC) Deployed."
