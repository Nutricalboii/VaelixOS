...
while true; do
    ...
    if [ \"\$AC_STATUS\" != \"\$(cat \$STATE_FILE 2>/dev/null)\" ]; then
        if [ \"\$AC_STATUS\" == \"1\" ]; then
            # AC CONNECTED: Premium Experience
            kwriteconfig5 --file kwinrc --group Compositing --key Enabled true
            kwriteconfig5 --file kwinrc --group Compositing --key AnimationSpeed 2
            # Reconfigure KWin
            qdbus org.kde.KWin /KWin reconfigure
            # Notify
            notify-send -a \"Vaelix Power\" \"Headquarters Powered\" \"Switching to Premium Experience\" -i battery-charging
        else
            # BATTERY: Efficiency Experience
            kwriteconfig5 --file kwinrc --group Compositing --key Enabled true
            kwriteconfig5 --file kwinrc --group Compositing --key AnimationSpeed 4 # Slower animations = less CPU
            # Potential: disable heavy effects if we can target them via CLI
            qdbus org.kde.KWin /KWin reconfigure
            # Notify
            notify-send -a \"Vaelix Power\" \"Endurance Mode\" \"Optimizing for battery longevity\" -i battery-low
        fi
